  ...
  <body>
    ...
    <div class="container">
        ...
        <div class="panel panel-primary Myback">
          ...
          while( $row = mysqli_fetch_array( $result ) ){
            ...
            "<tr>
			 ...
			 <td>
				<form action='' method='GET'>
					<input type='submit' name=".$row['drug_id']." value='Edit'> | 
					<input type='submit' name='delete".$row['drug_id']."' value='Delete' onClick=\"return confirm('Are you sure you want to delete?')\"> | 
					<input type='submit' name='print".$row['drug_id']."' value='Print'>
				</form>
			</td>
            </tr>\n";
			// UPDATE CODE STARTS FROM HERE
				if(isset($_GET[$row['drug_id']])){
					...
					echo "Drug Id: <input type='text'  name='drug_id' value=".$row['drug_id'].">";
					echo "</br></br>";
					echo "Drug Name: <input type='text' name='drug_name' value=".$row['drug_name'].">";
					echo "</br></br>";
					echo "Drug Category: <input type='text' name='drug_category' value=".$row['drug_category'].">";
					echo "</br></br>";
					echo "Drug Company: <input type='text' name='drug_company' value=".$row['drug_company'].">";
					echo "</br></br>";
					echo "Drug Desp: <input type='text' name='drug_desp' value=".$row['drug_desp'].">";
					echo "</br></br>";
					echo "Drug Cost: <input type='text' name='drug_cost' value=".$row['drug_cost'].">";
					echo "</br></br>";
					...
					if(isset($_POST['submit'])){
						...
						$company = $_POST["drug_company"];
						$desp = $_POST["drug_desp"];
						$cost = $_POST["drug_cost"];

						$ssql = "UPDATE tb_drug SET drug_id='$id', drug_name='$name', drug_category='$category', drug_company='$company', drug_desp='$desp', drug_cost='$cost'
				...
				if(isset($_GET['delete'.$row['drug_id']])){
					$delete = "DELETE FROM tb_drug WHERE drug_id=".$row['drug_id']."";
			...
			// PDF STARTS FROM HERE
				if(isset($_GET['print'.$row['drug_id']])){
					...
					mywindow.document.write('<p>' + 'ID NAME CATEGORY COMPANY DESP COST'  + '</p>');
					mywindow.document.write(document.getElementById('a".$row['drug_id']."').innerHTML);
					mywindow.document.write('  |  ');
					mywindow.document.write(document.getElementById('b".$row['drug_id']."').innerHTML);
					mywindow.document.write('  |  ');
					mywindow.document.write(document.getElementById('c".$row['drug_id']."').innerHTML);
					mywindow.document.write('  |  ');
					mywindow.document.write(document.getElementById('d".$row['drug_id']."').innerHTML);
					mywindow.document.write('  |  ');
					mywindow.document.write(document.getElementById('d".$row['drug_id']."').innerHTML);
					mywindow.document.write('  |  ');
					mywindow.document.write(document.getElementById('d".$row['drug_id']."').innerHTML);
        ...
        ?>
				...
				<center><a href="drugprint.php"><button class="btn btn-default">Print Information</button></a></center>
            </div>
        </div>
        </div>
      </div>
