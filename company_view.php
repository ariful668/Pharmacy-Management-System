...
<html lang="en">
  ...
  <head>
	<?php
      ...
      $result = mysqli_query($connector,"SELECT * FROM tb_company ");
    ?>
  ...
  <body>
    ...
    <div class="container">
        ...
        <div class="panel panel-primary Myback">
            <div class="panel-heading panel-head"><h2><center>Company Table</center></h2></div>
			</br>
			</br>
          ...
          while( $row = mysqli_fetch_array( $result ) ){
            echo
            "<tr>
              <td id='a".$row['company_invoice_id']."'>{$row['company_invoice_id']}</td>
              <td id='b".$row['company_invoice_id']."'>{$row['company_name']}</td>
              <td id='c".$row['company_invoice_id']."'>{$row['company_address']}</td>
              <td id='d".$row['company_invoice_id']."'>{$row['company_phone_no']}</td>
			 <td>
				<form action='' method='GET'>
					<input type='submit' name=".$row['company_invoice_id']." value='Edit'> | 
					<input type='submit' name='delete".$row['company_invoice_id']."' value='Delete' onClick=\"return confirm('Are you sure you want to delete?')\"> | 
					<input type='submit' name='print".$row['company_invoice_id']."' value='Print'>
				</form>
			</td>
            </tr>\n";
			// UPDATE CODE STARTS FROM HERE
				if(isset($_GET[$row['company_invoice_id']])){
					...
					echo "Company Id: <input type='text'  name='company_invoice_id' value=".$row['company_invoice_id'].">";
					echo "</br></br>";
					echo "Company Name: <input type='text' name='company_name' value=".$row['company_name'].">";
					echo "</br></br>";
					echo "Company Address: <input type='text' name='company_address' value=".$row['company_address'].">";
					echo "</br></br>";
					echo "Company Contact: <input type='text' name='company_phone_no' value=".$row['company_phone_no'].">";
					echo "</br></br>";
					...
					if(isset($_POST['submit'])){
						$id = $_POST["company_invoice_id"];
						$name = $_POST["company_name"];
						$address = $_POST["company_address"];
						$contact = $_POST["company_phone_no"];

						$ssql = "UPDATE tb_company SET company_invoice_id='$id', company_name='$name', company_address='$address', company_phone_no='$contact'
						WHERE company_invoice_id=".$row['company_invoice_id']."";
				...
				if(isset($_GET['delete'.$row['company_invoice_id']])){
					$delete = "DELETE FROM tb_company WHERE company_invoice_id=".$row['company_invoice_id']."";
			...
			// PDF STARTS FROM HERE
				if(isset($_GET['print'.$row['company_invoice_id']])){
					...
					mywindow.document.write(document.getElementById('a".$row['company_invoice_id']."').innerHTML);
					mywindow.document.write('  |  ');
					mywindow.document.write(document.getElementById('b".$row['company_invoice_id']."').innerHTML);
					mywindow.document.write('  |  ');
					mywindow.document.write(document.getElementById('c".$row['company_invoice_id']."').innerHTML);
					mywindow.document.write('  |  ');
					mywindow.document.write(document.getElementById('d".$row['company_invoice_id']."').innerHTML);
