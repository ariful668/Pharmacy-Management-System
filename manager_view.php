...
<html lang="en">
  ...
  <?php
      ...
      $result = mysqli_query($connector,"SELECT * FROM tb_manager ");
      ?>
  ...
  <body>
    ...
    <div class="container">
        ...
        <div class="panel panel-primary Myback">
            <div class="panel-heading panel-head"><h2><center>Manager Table</center></h2></div>
			</br>
			</br>
		...
		<?php
          while( $row = mysqli_fetch_array( $result ) ){
            echo
            "<tr>
              <td id='a".$row['manager_id']."'>{$row['manager_id']}</td>
              <td id='b".$row['manager_id']."'>{$row['manager_name']}</td>
              <td id='c".$row['manager_id']."'>{$row['manager_address']}</td>
              <td id='d".$row['manager_id']."'>{$row['manager_phone_no']}</td>
			  <td>
				<form action='' method='GET'>
					<input type='submit' name=".$row['manager_id']." value='Edit'> | 
					<input type='submit' name='delete".$row['manager_id']."' value='Delete' onClick=\"return confirm('Are you sure you want to delete?')\"> | 
					<input type='submit' name='print".$row['manager_id']."' value='Print'>
				</form>
			</td>
			  
            </tr>\n";
			// UPDATE CODE STARTS FROM HERE
				if(isset($_GET[$row['manager_id']])){
					...
					echo "Manager Id: <input type='text'  name='manager_id' value=".$row['manager_id'].">";
					echo "</br></br>";
					echo "Manager Name: <input type='text' name='manager_name' value=".$row['manager_name'].">";
					echo "</br></br>";
					echo "Manager Address: <input type='text' name='manager_address' value=".$row['manager_address'].">";
					echo "</br></br>";
					echo "Manager Contact: <input type='text' name='manager_phone_no' value=".$row['manager_phone_no'].">";
					echo "</br></br>";
					...
					if(isset($_POST['submit'])){
						...
						$contact = $_POST["manager_phone_no"];

						$ssql = "UPDATE tb_manager SET manager_id='$id', manager_name='$name', manager_address='$address', manager_phone_no='$contact'
				...
				if(isset($_GET['delete'.$row['manager_id']])){
					$delete = "DELETE FROM tb_manager WHERE manager_id=".$row['manager_id']."";
			...
			// PDF STARTS FROM HERE
				if(isset($_GET['print'.$row['manager_id']])){
					...
					mywindow.document.write(document.getElementById('a".$row['manager_id']."').innerHTML);
					mywindow.document.write('  |  ');
					mywindow.document.write(document.getElementById('b".$row['manager_id']."').innerHTML);
					mywindow.document.write('  |  ');
					mywindow.document.write(document.getElementById('c".$row['manager_id']."').innerHTML);
					mywindow.document.write('  |  ');
					mywindow.document.write(document.getElementById('d".$row['manager_id']."').innerHTML);
        ...
        ?>
				...
				<center><a href="managerprint.php"><button class="btn btn-default">Print Information</button></a></center>
            </div>
        </div>
        </div>
      </div>
