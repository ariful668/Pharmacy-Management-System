...
<html lang="en">
  ...
  <?php
      ...
      $result = mysqli_query($connector,"SELECT * FROM tb_payment ");
      ?>
  ...
  <body>
    ...
    <div class="container">
        ...
        <div class="panel panel-primary Myback">
            <div class="panel-heading panel-head"><h2><center>Payment Table</center></h2></div>
			</br>
			</br>
          ...
          while( $row = mysqli_fetch_array( $result ) ){
            echo
            "<tr>
              <td id='a".$row['payment_id']."'>{$row['payment_id']}</td>
              <td id='b".$row['payment_id']."'>{$row['payment_name']}</td>
              <td>
				<form action='' method='GET'>
					<input type='submit' name=".$row['payment_id']." value='Edit'> | 
					<input type='submit' name='delete".$row['payment_id']."' value='Delete' onClick=\"return confirm('Are you sure you want to delete?')\"> | 
					<input type='submit' name='print".$row['payment_id']."' value='Print'>
				</form>
			</td>
            </tr>\n";
			// UPDATE CODE STARTS FROM HERE
				if(isset($_GET[$row['payment_id']])){
					...
					echo "Payment Id: <input type='text'  name='payment_id' value=".$row['payment_id'].">";
					echo "</br></br>";
					echo "Payment Name: <input type='text' name='payment_name' value=".$row['payment_name'].">";
					echo "</br></br>";
					...
					if(isset($_POST['submit'])){
						...
						$ssql = "UPDATE tb_payment SET payment_id='$id', payment_name='$name'
				...
				if(isset($_GET['delete'.$row['payment_id']])){
					$delete = "DELETE FROM tb_payment WHERE payment_id=".$row['payment_id']."";
			...
			// PDF STARTS FROM HERE
				if(isset($_GET['print'.$row['payment_id']])){
					...
					mywindow.document.write('<p>' + 'ID NAME'  + '</p>');
					mywindow.document.write(document.getElementById('a".$row['payment_id']."').innerHTML);
					mywindow.document.write('  |  ');
					mywindow.document.write(document.getElementById('b".$row['payment_id']."').innerHTML);
        ...
        ?>
				...
				<center><a href="paymentprint.php" ><button class="btn btn-default">Print Information</button></a></center>
            </div>
        </div>
        </div>
      </div>
